...
export const getSingleTaxBrackets = (year: TaxYear) => {
  switch (year) {
    case '2023': return SINGLE_TAX_BRACKETS_2023;
    case '2024': return SINGLE_TAX_BRACKETS_2024;
    case '2025': return SINGLE_TAX_BRACKETS_2025;
    default: return SINGLE_TAX_BRACKETS_2023;
  }
};

export const getMarriedTaxBrackets = (year: TaxYear) => {
  switch (year) {
    case '2023': return MARRIED_TAX_BRACKETS_2023;
    case '2024': return MARRIED_TAX_BRACKETS_2024;
    case '2025': return MARRIED_TAX_BRACKETS_2025;
    default: return MARRIED_TAX_BRACKETS_2023;
  }
};

// Calculate tax based on taxable income and brackets
export const calculateTaxFromBrackets = (taxableIncome: number, brackets: typeof SINGLE_TAX_BRACKETS_2023): number => {
  let tax = 0;
...
export const calculateSingleTax = (person: TaxPerson, year: TaxYear): number => {
  const standardDeduction = STANDARD_DEDUCTION[year].single;
  
  // Calculate total itemized deductions
  const totalItemizedDeductions = getTotalItemizedDeductions(
    person.itemizedDeductions || { salt: 0, mortgageInterest: 0, mortgageLoanAmount: 0, other: 0 },
    year,
    'single'
  );
...
export const calculateMarriedTax = (
  ...
  const combinedItemizedDeductions: ItemizedDeductions = {
    salt: (person1.itemizedDeductions?.salt || 0) + (person2.itemizedDeductions?.salt || 0),
    mortgageInterest: (person1.itemizedDeductions?.mortgageInterest || 0) + (person2.itemizedDeductions?.mortgageInterest || 0),
    mortgageLoanAmount: (person1.itemizedDeductions?.mortgageLoanAmount || 0) + (person2.itemizedDeductions?.mortgageLoanAmount || 0),
    other: (person1.itemizedDeductions?.other || 0) + (person2.itemizedDeductions?.other || 0)
  };
  ...
  const taxableIncomeWithStandard = Math.max(0, combinedIncome - standardDeduction);
  const taxWithStandard = calculateTaxFromBrackets(taxableIncomeWithStandard, getMarriedTaxBrackets(year));
  
  // Calculate tax using itemized deductions
  const taxableIncomeWithItemized = Math.max(0, combinedIncome - totalItemizedDeductions);
  const taxWithItemized = calculateTaxFromBrackets(taxableIncomeWithItemized, getMarriedTaxBrackets(year));
...
export const calculateTaxComparison = (
  ...
  const { tax: marriedTax, optimalFilingMethod } = calculateMarriedTax(person1, person2, year);
  
  const difference = combinedSingleTax - marriedTax;
  const percentageDifference = combinedSingleTax > 0 
    ? (difference / combinedSingleTax) * 100 
    : 0;
